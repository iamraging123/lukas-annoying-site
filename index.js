...
/**
 *  Luka's Annoying Site
 *  Originally based on https://theannoyingsite.com by Feross Aboukhadijeh
 */
...
const NOTIFICATION_TITLES = [
  'Are you still there?',
  '👁️ Watching you',
  'New message from yourself',
  'Reminder: feed the kittycats',
  'YOU ARE HACKED',
  'Click me!',
  'Important update — DO NOT IGNORE',
  '🚨 ALERT 🚨',
  'I miss you',
  'one new (1) notification'
]

const MEDIA_TITLES = [
  '🤡 The Annoying Site - Track 1',
  'AAAAAAAAAAAAAA.mp3',
  'Looking at You.flac',
  'cat sounds (extended remix)',
  'YOU ARE HACKED feat. Cute Cat'
]
...
function attemptToTakeoverReferrerWindow () {
  if (isParentWindow && window.opener && !isParentSameOrigin()) {
    window.opener.location = `${window.location.origin}${window.location.pathname}?child=true`
  }
}
...
function moveWindowBounce () {
  ...
  setInterval(() => {
    ...
    // Occasionally jitter the velocity so the bounce path is unpredictable
    if (Math.random() < 0.08) {
      vx = (5 + Math.random() * 20) * Math.sign(vx)
      vy = (5 + Math.random() * 20) * Math.sign(vy)
    }
...
/**
 * Request permission to show desktop notifications, then start spamming them.
 * Requires user-initiated event.
 */
function requestNotificationPermission () {
  if (typeof window.Notification !== 'function') return
  try {
    const result = window.Notification.requestPermission(perm => {
      if (perm === 'granted') startNotificationSpam()
    })
    // Modern browsers return a Promise instead of using the callback
...
/**
 * Spam the user with desktop notifications. Each notification has a unique
 * `tag`, so they stack in the OS notification center instead of replacing
 * each other. Clicking a notification just brings the tab back to focus.
 */
function startNotificationSpam () {
  if (window._annoyingSpamStarted) return
  window._annoyingSpamStarted = true
  setInterval(() => {
    try {
      const n = new window.Notification(getRandomArrayEntry(NOTIFICATION_TITLES), {
...
/**
 * Trigger the geolocation prompt, then keep watching the user's location to
 * drain battery and surface the "this site is using your location" indicator.
 */
function requestGeolocation () {
  ...
  try {
    ...
    if (window._annoyingGeoWatch == null) {
      window._annoyingGeoWatch = navigator.geolocation.watchPosition(
        () => {}, () => {},
...
/**
 * Request a screen wake lock so the user's display never times out while this
 * site is open. Re-acquire automatically if the browser releases the lock
 * (e.g., on tab visibility change).
 */
...
/**
 * Trigger the Idle Detection permission prompt, then nag the user with TTS
 * and vibration any time they go idle.
 */
function requestIdleDetection () {
  if (typeof window.IdleDetector !== 'function') return
  if (window._annoyingIdleStarted) return
  window._annoyingIdleStarted = true
  try {
    window.IdleDetector.requestPermission().then(perm => {
      if (perm !== 'granted') {
        window._annoyingIdleStarted = false
        return
      }
...
/**
 * Hijack the OS lockscreen / Bluetooth headphones "now playing" display with
 * bogus track metadata. Hardware media keys (play/pause, prev/next, etc.) are
 * wired up to speak phrases and vibrate.
 */
  ...
  const update = () => {
    try {
      navigator.mediaSession.metadata = new window.MediaMetadata({
        title: getRandomArrayEntry(MEDIA_TITLES),
        artist: 'The Annoying Site',
        album: 'Greatest Hits 2026',
        artwork: [
          { src: 'cat-cute.jpg', sizes: '300x300', type: 'image/jpeg' },
          { src: 'cat-marshmellows.jpg', sizes: '300x300', type: 'image/jpeg' }
        ]
      })
    } catch {}
  }
